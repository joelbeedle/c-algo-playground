...
Test(linked_list_tests, test_createNode) {
    ...
    cr_assert_eq(*(int*)result->data.data, value, "Data value mismatch in created node");
...
Test(linked_list_tests, test_llist_append) {
    ...
    for (int i = 0; i < num_values; ++i) {
        ...
        for (int j = 0; j <= i; ++j) {
            ...
            cr_assert_eq(*(int*)current->data.data, values[j], "Data value mismatch in node");
...
Test(linked_list_tests, test_llist_fromarr) {
    ...
    for (int i = 0; i < num_values; ++i) {
        cr_assert_not_null(current, "Node is unexpectedly NULL");
        cr_assert_eq(*(int*)current->data.data, values[i], "Data value mismatch in node");
        current = current->next;
    }
...
Test(linked_list_tests, test_llist_delete) {
    ...
    Node* current = list->head;
    for (int i = 0; i < num_values; ++i) {
        cr_assert_not_null(current, "Node is unexpectedly NULL after failed delete");
        cr_assert_eq(*(int*)current->data.data, values[i], "Data value mismatch in node after failed delete");
        current = current->next;
    }
    ...
    for (int i = 0; i < num_values - 1; ++i) {
        ...
        cr_assert_eq(*(int*)current->data.data, expected_remaining_values[i], "Data value mismatch in node after deletion");
        current = current->next;
    }
